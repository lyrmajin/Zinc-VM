...
uint8_t FL = 0;     // RUNTIME SYSTEM 0 0 0 0 0 CMP
...
const SYSCALL SYSCALLS[11] = {SysWriteIO, SysReadIO, SysScheduleTask, SysKillTask, SysMalloc, SysFree, SysGetPC, SysHalt, SysKill, SysOpen, SysRead, SysExec};
...
void OP_LD() { //1000
 ...
 if((MEMORY[REGISTER[PC]] & 0b00000100) == 0b00000000)
   ...
   address = ((0x100 + (currentTask * 2)) + (REGISTER[(MEMORY[REGISTER[PC] + 1] & 0b11000000) >> 0x06]));
 if (address <= (0x101 + (currentTask * 2)))
...
uint16_t address = 0;
 if((MEMORY[REGISTER[PC]] & 0b00000100) == 0b00000000)
   ...
   address = (((0x100 + (currentTask * 2)) + (REGISTER[((MEMORY[REGISTER[PC] + 1] & 0b11000000) >> 0x06)] + (MEMORY[REGISTER[PC] + 1] & 0b00111111))));
 if (address <= (0x101 + (currentTask * 2)))
...
void storeTask() {
 if (MEMORY[currentTask]) {
   ...
   MEMORY[MEMORY[currentTask] + 10] = FL;
 }
}
void loadTask() {
 if (MEMORY[currentTask]) {
   ...
   FL = MEMORY[MEMORY[currentTask] + 10];
 }
}
