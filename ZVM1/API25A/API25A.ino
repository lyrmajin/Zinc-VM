...
//----------------------------------------------
void SysWriteIO(uint8_t argptr = 0, volatile uint8_t* port = 0, uint8_t value = 0);
uint8_t SysReadIO(uint8_t argptr = 0, volatile uint8_t* port = 0);
// 2
uint8_t SysScheduleTask(uint8_t argptr = 0, uint8_t adrs = 0);
// 3
...
// 6
uint8_t SysOpen(uint8_t argptr = 0, char *file = "", uint16_t baseadrs = 0x300);
uint8_t SysRead(uint8_t argptr = 0, uint16_t baseadrs = 0x300, uint8_t offset = 0);
// 8
void SysStore(uint8_t argptr = 0, uint16_t address = 0, uint8_t value = 0);
void SysMove(uint8_t argptr = 0, uint16_t address = 0, uint16_t address_b = 0);
uint8_t SysLoad(uint8_t argptr = 0, uint16_t address = 0);
// 9
void SysKillTask(uint8_t argptr = 0, uint8_t taskid = 0);
void SysKill(uint8_t argptr = 0);
// B
uint8_t SysExec(uint8_t argptr = 0, uint16_t address = 0);

typedef void (*SYSCALL)(uint8_t);
const SYSCALL SYSCALLS[14] = {SysWriteIO, SysReadIO, SysScheduleTask, SysMalloc, SysFree, SysGetPC, SysOpen, SysRead, SysStore, SysMove, SysLoad, SysKillTask, SysKill, SysExec};

void OP_MVI() { // 0000
  REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)] = MEMORY[REGISTER_PC + 1];
  REGISTER_PC += 2;
}
void OP_MOV() { // 0001
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] = REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  REGISTER_PC++;
}
void OP_ADD() { //0010
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] += REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
...
void OP_CMP() { //0111
  FL &= 0b11111100;
  if ((REGISTER[((MEMORY[REGISTER_PC] & 0b00001100) >> 0x02)] - REGISTER[((MEMORY[REGISTER_PC] & 0b00000011))]) == 0)
...
void OP_LD() { //1000
  MEMORY[REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02]] = REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  REGISTER_PC++;
}
void OP_ST() { //1001
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] = MEMORY[REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)]];
  REGISTER_PC++;
}
void OP_JMP() { //1010
  if (((MEMORY[REGISTER_PC] & 0b00001100) == 0b00000000))
    REGISTER_PC = REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  else if (((MEMORY[REGISTER_PC] & 0b00001100) == 0b00000100))
    REGISTER_PC += REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  else if (((MEMORY[REGISTER_PC] & 0b00001100) == 0b00001000))
    REGISTER_PC -= REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
}
void OP_BRN() {
  if (((MEMORY[REGISTER_PC] & 0b00000100) == 0b00000000 && (FL & 0b00000001)) || ((MEMORY[REGISTER_PC] & 0b00000100) == 0b00000100 && (FL & 0b00000010)))
  {
    if (((MEMORY[REGISTER_PC] & 0b00001000) == 0b00000000))
      REGISTER_PC = REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
    else if (((MEMORY[REGISTER_PC] & 0b00001000) == 0b00001000))
      REGISTER_PC += REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  }
}
void OP_SHL() { //1100
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] = REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] << REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
  REGISTER_PC++;
}
void OP_SHR() { //1101
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] = REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] >> REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)];
...
void OP_SCB() { //1110
  REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02] ^= (1 << REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)]);
  REGISTER_PC++;
}
void OP_SYS() { //1111
  SYSCALLS[REGISTER[(MEMORY[REGISTER_PC] & 0b00001100) >> 0x02]](REGISTER[(MEMORY[REGISTER_PC] & 0b00000011)]);
  REGISTER_PC++;
}
typedef void (*OPCODE)();
...
void storeTask() {
  if (MEMORY[currentTask]) {
    ...
    MEMORY[MEMORY[currentTask] + 4] = REGISTER_PC;
...
void loadTask() {
  if (MEMORY[currentTask]) {
    ...
    REGISTER_PC = MEMORY[MEMORY[currentTask] + 4];
...
void VMloop(boolean DEBUG = false) {
  while ((FL & 0b10000000)) {
    OPCODES[((MEMORY[REGISTER_PC] & 0b11110000) >> 0x04)]();
    ...
    if (DEBUG) {
      ...
      Serial.println(MEMORY[REGISTER_PC], HEX);
...
void SysWriteIO(uint8_t argptr, volatile uint8_t* port, uint8_t value) {
  if (argptr) {
    port = (volatile uint8_t*)(MEMORY[argptr] | (MEMORY[argptr + 1] << 8));
    value = MEMORY[argptr + 2];
  }
  *port = value;
}

uint8_t SysReadIO(uint8_t argptr, volatile uint8_t* port) {
  if (argptr)
    port = (volatile uint8_t*)(MEMORY[argptr] | (MEMORY[argptr + 1] << 8));
  REGISTER[3] = *port;
  return REGISTER[3];
...
uint8_t SysScheduleTask(uint8_t argptr, uint8_t adrs) {
  if (argptr)
    adrs = MEMORY[argptr];
...
void SysKill(uint8_t argptr) {
  FL &= 0b01111111;
}

void SysStore(uint8_t argptr, uint16_t address, uint8_t value) {
  if (argptr) {
    address = (MEMORY[argptr] << 0x08) | MEMORY[argptr + 1];
    value = MEMORY[argptr + 2];
  }
  MEMORY[address] = value;
}

void SysMove(uint8_t argptr, uint16_t address, uint16_t address_b) {
  if (argptr) {
    address = (MEMORY[argptr] << 0x08) | MEMORY[argptr + 1];
    address_b = (MEMORY[argptr + 2] << 0x08) | MEMORY[argptr + 3];
  }
  MEMORY[address] = address_b;
}

uint8_t SysLoad(uint8_t argptr, uint16_t address) {
  if (argptr)
    address = (MEMORY[argptr] << 0x08) | MEMORY[argptr + 1];
  REGISTER[3] = MEMORY[address];
  return REGISTER[3];
}

uint8_t SysExec(uint8_t argptr, uint16_t address) {
  if (argptr) {
    address = (MEMORY[argptr] << 8) | (MEMORY[argptr + 1]);
  }
  SysScheduleTask(0, MEMORY[0xFF]);
  REGISTER_PC = address + 1;
}
